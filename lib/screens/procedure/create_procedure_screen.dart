  ...
  Future<void> _createProcedure() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Procedure created successfully.')),
        );
        Navigator.pop(context);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to create procedure. Please try again.')),
        );
      }
    } catch (e) {
            ...
            child: Column(
              children: [
                ...
                TextField(
                    ...
                    style: const TextStyle(color: Colors.white, fontSize: 16),
                    decoration: InputDecoration(
                        hintText: 'Procedure Date (YYYY-MM-DD)',
                        filled: true,
                        fillColor: MainColors.textfield,
                        hintStyle: const TextStyle(color: MainColors.hinttext, fontSize: 16),
                        border: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: BorderSide.none),
